...
int cur_time() {
  char readBuffer[16];  // to read magic value DONE + null
  //
  // Send the Remote Call
  //
  c150debug->printf(C150RPCDEBUG,"simplefunction.proxy.cpp: cur_time() invoked");
  RPCPROXYSOCKET->write("cur_time-", strlen("cur_time-")+1); // write function name including null
  //
  // Read the response
  //
  c150debug->printf(C150RPCDEBUG,"simplefunction.proxy.cpp: cur_time() invocation sent, waiting for response");
  RPCPROXYSOCKET->read(readBuffer, sizeof(readBuffer)); // only legal response is DONE

  //check and make sure the response code is OK
  // Check the response
  ...
  //pick out the number itself

  return atoi(&(readBuffer[4]));
  //return said number
...
bool is_prime() {
  ...
  // Send the Remote Call
  //
  c150debug->printf(C150RPCDEBUG,"simplefunction.proxy.cpp: is_prime() invoked");

  //gotta pass parameter

  RPCPROXYSOCKET->write("is_prime-", strlen("is_prime-")+1); // write function name including null
  //
  // Read the response
  //
  c150debug->printf(C150RPCDEBUG,"simplefunction.proxy.cpp: is_prime() invocation sent, waiting for response");
  RPCPROXYSOCKET->read(readBuffer, sizeof(readBuffer)); // only legal response is DONE
