...
void __cur_time() {
  char doneBuffer[16] = "DONE";  // to write magic value DONE + null
  int val;
  ...
  //pack val into a string
  char buffer[11];
  sprintf(buffer, "%06d", val);
  memcpy(&doneBuffer[4], buffer, 11);
...
void dispatchFunction() {
  ...
  if (!RPCSTUBSOCKET-> eof()) {
    if (strcmp(functionNameBuffer,"func1") == 0)
      ...
      __func3();
    else   if (strcmp(functionNameBuffer,"cur_time") == 0)
      __cur_time();
    else
